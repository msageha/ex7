...
int main( int argc, char* argv[] ) {
  ...
  struct player_t* op_player;
  if(argc<1) {
    fprintf( stderr, "Usage: %s remote HOSTNAME SERVICE (or random)\n", argv[0]);
    return 0;
  }
  
  if(strcmp(argv[1], "random")==0) {
    op_player = make_random_player();
  }else if(strcmp(argv[1], "remote")==0) {
    if ( argc < 4 ) {
      fprintf( stderr, "Usage: %s remote HOSTNAME SERVICE (or random)\n", argv[0]);
      return 0;
    }
    op_player = make_remote_player(argv[2], argv[3]);
  }else{
    fprintf( stderr, "Usage: %s remote HOSTNAME SERVICE (or random)\n", argv[0]);
    return 0;
  }
